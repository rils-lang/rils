...
- `execute_with_limit` 提供确定的指令步数预算，防止脚本无限占用执行线程。
- 调用栈默认限制为 1024 帧，递归不会消耗 Rust 宿主调用栈直至进程溢出。
...
运行时指令。模块内 impl、关联函数、成员调用和模块内 UFCS 使用同一限定符号表。`compile_file`
会按照解释器相同的规则递归加载 `name.rils` 或 `name/mod.rils`，再将完整程序编译为一个模块。
...
`std::io` 与 `std::fs` 的实现可分别启用，并受同名 capability 控制。自定义
...
| 标准库/宿主 | 部分支持 | core/Vec、内置 print/println/assert 宏与显式授权的 std::io/std::fs 已链接；Engine 宿主注册整合待完成 |
| 磁盘预编译 | 实验可用 | `.rilbc` v1、bytes/file API、CLI compile/verify/run；尚未承诺跨版本稳定 |
...
HashMap/HashSet，以及 Engine 自定义宿主函数直接编译。运行时错误使用
`BytecodeError`，同样可以通过 `render`
生成带源码位置的诊断。

## 磁盘格式

当前已实现实验性 `.rilbc` v1。它采用带版本的显式小端容器，不直接序列化任何 Rust enum、地址或
内存布局：
...
v1 包含 module、imports、types、iterators 和 functions 五个必需 section；常量、指令和源码 Span
使用各自的显式 tag/字段编码。加载器限制文件为 64 MiB、单个字符串为 1 MiB、通用集合为一百万项、
...
由于语言当前存在 `usize`/`isize`，v1 记录目标指针宽度，32 位和 64 位产物不允许交叉加载，避免
发生静默截断。`format version`、`language version` 和 `host ABI` 分别检查。当前格式仍处于 0.1.0
...
`compile` 仍不访问文件；CLI 和 `compile_file` 采用统一的递归模块加载规则，将入口和所有外部模块
...
内存模块已经只记录稳定导入符号、函数签名、能力类别和 ABI 版本，不记录进程地址；缺失、签名
不一致、ABI 不匹配或未授权都会在执行前成为链接错误。后续把 Engine 注册的游戏 API 接入同一
宿主表，并为磁盘加载器固化相同验证边界。

### 2. `.rilbc` 容器收尾

v1 编码/加载 API、CLI 和严格加载验证已经完成。后续增加多文件文件表，让每条指令同时携带文件
身份与 span；再拆出可剥离调试 section，并补充跨目标构建矩阵与格式 fuzz 测试。

### 3. 覆盖收尾与优化

继续补齐嵌套 place、Vec 动态借用和借用迭代器。随后建立基准，按数据加入常量折叠、
无效代码删除、分支简化、寄存器复用与热点统计，并为堆内存、容器长度、字符串长度、宿主调用次数
增加独立预算。最后以 MIR 为共同输入探索等价 Rust AOT；它不改变语言语义，也不绕过宿主 ABI。

更完整的优先级、依赖和验收标准见 [implementation-roadmap.md](implementation-roadmap.md)。
