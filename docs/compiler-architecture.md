...
5. AST 解释器消费共享 `TypeckResults`，配置项目入口使用项目 `DefMap` 并以 frontend error diagnostic 作为执行前 gate；frontend 在项目级按模块路径解析 trait 声明与 `use` 导入，将完成 supertrait、associated type 声明和方法契约检查的 impl 记录为稳定 `ImplId`，项目解释器对这些 impl 复用验证结果。associated type 检查覆盖缺失成员、额外成员和泛型参数数量，普通 `eval` 或无法由项目语义解析的动态路径继续保留运行时防御。剩余类型兼容检查和名称查找收缩属于后续独立迭代。
