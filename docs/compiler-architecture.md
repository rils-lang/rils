...
6. 进行中：数值、HashMap/HashSet、String、Vec、Option/Result，以及内建 `SequenceIterator` 的无 callback 操作已共享 runtime dispatcher；仅保留需要调用 Rils 函数值的 Option/Result 和自定义 Iterator callback adapter。下一步将 dispatcher 下沉为后端中立模块，并继续评估自定义 iterator 的默认方法边界。
