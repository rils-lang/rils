...
ENUM_RE = re.compile(
    r"enum\s+(RilsStatus|RilsValueTag|RilsHostTypeKind|RilsFormatKind)\s*\{(.*?)\};", re.DOTALL
)
...
def parameter(c_parameter: str) -> str:
    ...
    patterns = [
        ...
        (r"RilsSlice (\w+)", lambda name: f"NativeSlice {name}"),
        (r"RilsValue (\w+)", lambda name: f"NativeValue {name}"),
        (r"RilsHostDispatcher (\w+)", lambda name: f"NativeHostDispatcher {name}"),
        (r"RilsOutputCallback (\w+)", lambda name: f"NativeOutputCallback? {name}"),
        (r"RilsHostValueFormatCallback (\w+)", lambda name: f"NativeHostValueFormatCallback? {name}"),
        (r"void \*(\w+)", lambda name: f"IntPtr {name}"),
        (r"uint64_t (\w+)", lambda name: f"ulong {name}"),
        (r"uint32_t (\w+)", lambda name: f"uint {name}"),
...
def generate(header: str) -> str:
    ...
    type_kinds = enum_source(header, "RilsHostTypeKind", "RILS_HOST_TYPE_", "uint")
    format_kinds = enum_source(header, "RilsFormatKind", "RILS_FORMAT_", "uint")
