...
- 新建或扩展 crate/工具时，应从一开始按职责安排子模块；`lib.rs`/`main.rs` 默认作为转发层，可保留跨模块通用的小型辅助函数或对子模块的简单封装，但不得承载具体领域流程、配置解析或文件处理实现。
- crate 的测试应优先放在其自身的 `tests/` 目录，不依赖主 crate 的测试目录；涉及项目配置、文件扫描或输入样例的测试，应将可复用用例作为 `tests/fixtures/` 下留存的真实文件，而非在测试代码中临时拼接全部输入。仅适合纯函数的极小单元测试可例外。

# Git 与提交规范

- 开发采用“版本分支 -> feature 分支 -> 版本分支 -> 主分支”的流转方式：每个 feature 分支必须从当前版本分支拉出，开发完成后合并回对应版本分支；不得绕过版本分支直接合并到主分支。
- 当前版本的全部迭代完成并通过发布前验证后，才将版本分支合并回主分支；发包必须在该合并完成后进行。
...
- 正式发版前必须核对当前版本的 `CHANGELOG.md`：以实际发版基线和提交历史检查每项用户可见变更，
  仅将发版前已完成的内容归档到当前版本；发版后新增的能力、修复和兼容性调整必须继续保留在
  `Unreleased`，不得因为整理版本号而一并归档。
...
- 当前不实现生命周期参数或引用生命周期提升。引用不能被闭包捕获或成为全局绑定；来自函数参数的引用可以返回，来自局部值的引用不能返回。
- 引用可以在词法作用域内存入 tuple、数组、Option、Result、Vec、HashMap value 和泛型 struct/enum 实例，但容器不能跨越引用来源的作用域。
- Struct/enum 字段默认持有 `T` 的所有权；禁止直接声明 `&T` / `&mut T` 字段，泛型字段实例可以携带受词法作用域约束的引用。
...
- 参考 Rust 基础类型 API 时必须适配 Rils 的词法引用约束；引用返回或进入容器时必须保留来源和作用域，不能返回局部值的引用或提升生命周期。必要时提供 Copy、Clone 或消费式替代接口。
