...
- `.rilbc` v8 增加标准库原生符号导入表和 `CallNative` 指令，
  ...
  旧 v8 文件若引用已移除的 ID，也需从源码重新生成 `.rilbc`、
...
- Rust 宿主若直接构造 `BuiltinDeclaration`，需填写新增的 `native_symbol` 字段；
  直接构造 `NativeFunction` 的代码需将 `function` 字段改为 `body: NativeFunctionBody::Rust(...)`。
...
- trait impl 的 associated type 契约现在由共享 frontend 检查，覆盖缺失成员、额外成员和泛型参数
  数量不一致；配置项目的解释器与字节码编译器会在执行前返回一致诊断。
