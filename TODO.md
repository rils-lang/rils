...
- 将 VecDeque 和 BinaryHeap 接入借用迭代器，补齐 `iter_mut()`；让 `take/skip/enumerate/map/filter` 等适配器在调用 `next()` 时再计算元素。目前拥有型数组和 Vec 已按需取值，适配器和字符串迭代仍会预先收集结果。
...
- 继续把标准库的 `.rils` 占位声明迁移到 `rils_stdlib` 的 Rust 定义，并让 `BUILTINS`、语言包源码和 Analyzer 均直接消费生成元信息；迁移完成后删除重复声明与旧的按 ID 手写实现。
